...
struct Members //contient une table de tous les membres ainsi que des fonctions pour edit/lire celle-ci
{
  ...
  public:                                         //On retourne au public, les éléments seront à nouveau appelables en dehors de l'instance de la classe
    ...
    void save(std::string path = "save/members.irdl");  //cette fonction sauvegarde la table dans un fichier externe
    void load(std::string path = "save/members.irdl", char delimiter = '/', char end_line = '\\');  //cette fonction charge la table depuis un fichier externe
